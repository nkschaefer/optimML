...
To accomodate these constraints, each variable is logit transformed and divided by the sum of all mixture component variables. In other words, if the initial guess for a given mixture component variable $m_j$ is $g_j$, then each $m_j$ is initialized to $m_j = \frac{1}{e^{-\frac{g_j}{\sum\limits_{x=1}^3 g_x} + 1}$. each mixture component variable $m_j$ is  in the log likelihood function as follows: $$t(m_j) = \frac{\frac{1}{e^{-m_j} + 1}}{\sum\limits_{k=1}^n \frac{1}{e^{-m_k} + 1}}$$
