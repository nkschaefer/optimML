 ...
 * a truncated normal
 */
double mixcomp_solver::y_norm(vector<double>& x, map<string, double>& params_d, 
    map<string, int>& params_i){
    return -0.5*pow((x[0]-params_d["mu"])/params_d["sigma"], 2) - log(params_d["sigma"]) - 
        log(sqrt(2*M_PI));
}
 ...
 * Log likelihood Beta
 */
double mixcomp_solver::y_beta(vector<double>& x, map<string, double>& params_d,
    map<string, int>& params_i){
 ...
 */
double mixcomp_solver::y_binom(vector<double>& x, map<string, double>& params_d,
    map<string, int>& params_i){
    double n = params_d["n"];
    double k = params_d["k"];
